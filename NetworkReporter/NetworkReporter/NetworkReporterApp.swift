...
struct NetworkReporterApp: App {
    ...
    @StateObject private var xpcClient = XPCClient(persistenceController: PersistenceController.shared)

    init() {
        // Register a default value for monitoringInterval if it's not already set.
        if UserDefaults.standard.double(forKey: "monitoringInterval") == 0.0 {
            UserDefaults.standard.set(5.0, forKey: "monitoringInterval")
        }
    }
